...
from book
group by amount
--группирует по количеству экземпляров каждого произведения 
having count(amount)=1)
--выводит только те кол-ва книг, которые встречаются по одному разу, т.е. количество экземпляров которых в таблице book не дублируется.
+---------------+-----------------------+--------+
| author        | title                 | amount |
+---------------+-----------------------+--------+
