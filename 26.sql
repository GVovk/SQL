...
--аказ высчитывается на уровне подзапроса
from book
where amount <> (select max(amount) from book)
-- выбираем только те кол-ва книг которые не равны макс, т.е.  новые не нужны в заказе
--Результат
+--------------------+------------------+--------+-------+
...
--!!!В основном запросе нет группировки, поэтому MAX() использовать нельзя. Если же добавить группировку по title, author, amount, то нельзя будет посчитать MAX  по всей таблице.
--!!(SELECT (max(amount)-amount) FROM book) --> (SELECT max(amount) FROM book) -amount, так будет верно, так как amount вычитается на уровне основного запроса.